...
json YaScheduleApp::getScheduleBetweenCities() {
	...
	std::map <std::string, std::string> transportCompanyCode;
	std::map <std::string, std::string> vehicleModel;
	std::map <std::string, std::string> departureTimeAndArrivalTime;
	std::map <std::string, std::string> AreThereAnyTransfers;
	std::map <std::string, std::string> flightPrice;
	for (int i = 0; i < result["segments"].size(); i++){
		...
		race.push_back(transportCompanyCode);
		race.push_back(vehicleModel);
		race.push_back(departureTimeAndArrivalTime);
		race.push_back(AreThereAnyTransfers);
		race.push_back(flightPrice);
